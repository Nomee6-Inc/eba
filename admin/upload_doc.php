...
if(isset($_POST["submit"])) {
  $getdocname = $_POST['docname'];
  $md5docname = md5($getdocname);
  $uploaddir = '../docs/';
  $uploadfile = $uploaddir . "$md5docname.pdf";
	if (move_uploaded_file($_FILES['docupload']['tmp_name'], $uploadfile)) {
        echo "Döküman başarıyla yüklendi. <br>";
	} else {
		echo "Döküman yüklenirken hata oluştu! <br>";
	};
  $uploaddirt = '../assets/';
  $uploadfilet = $uploaddirt . basename($_FILES["thumbnail"]["name"]);
	...
	function fileWriteAppend(){
		$current_data = file_get_contents('../docs.json');
		$array_data = json_decode($current_data, true);
		$extra = array(
			 'docid'               =>     md5($_POST['docname']),
             'name'          =>     $_POST['docname'],
...
if(file_exists('../docs.json'))
{
     ...
     if(file_put_contents('../docs.json', $final_data))
     {
          $message = "success";
     }
}
							...
							<div class="col-xl-4">
				            	<div class="row">
									<div class="col-md-6 col-xl-12">
										<div class="mb-3">
                                            <input type="text" placeholder="Döküman İsmi" name="docname" id="docname" class="form-control" required>
                                        </div>
                                        <div class="mb-3">
                                            <div class="form-label">Döküman</div>
                                            <input type="file" accept=".pdf" name="docupload" id="docupload" class="form-control" required>
                                        </div>
                                        <div class="mb-3">
                                            <div class="form-label">Döküman Kapağı</div>
