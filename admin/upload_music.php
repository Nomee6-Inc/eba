...
if(isset($_POST["submit"])) {
  ...
  $uploadfile = $uploaddir . "$md5musicname.mp3";
	if (move_uploaded_file($_FILES['musicupload']['tmp_name'], $uploadfile)) {
        echo "Müzik başarıyla yüklendi. <br>";
	} else {
		echo "Müzik yüklenirken hata oluştu! <br>";
	};
  $uploaddirt = '../assets/';
  $uploadfilet = $uploaddirt . basename($_FILES["thumbnail"]["name"]);
	if (move_uploaded_file($_FILES['thumbnail']['tmp_name'], $uploadfilet)) {
        echo "Thumbnail başarıyla yüklendi.";
	} else {
		echo "Thumbnail yüklenirken hata oluştu!";
	};
	function fileWriteAppend(){
		$current_data = file_get_contents('../musics.json');
		$array_data = json_decode($current_data, true);
		$extra = array(
			 'musicid'               =>     md5($_POST['musicname']),
...
if(file_exists('../musics.json'))
{
     ...
     if(file_put_contents('../musics.json', $final_data))
     {
          $message = "success";
     }
}
