...
if(isset($_POST["submit"])) {
  $getmusicname = $_POST['musicname'];
  $md5musicname = md5($getmusicname);
  $uploaddir = '../musics/';
  $uploadfile = $uploaddir . "$md5musicname.mp3";
  $uploaddirt = '../assets/';
  $uploadfilet = $uploaddirt . basename($_FILES["thumbnail"]["name"]);
  $getthumbnailname = basename($_FILES["thumbnail"]["name"]);
  
	if (move_uploaded_file($_FILES['musicupload']['tmp_name'], $uploadfile) && move_uploaded_file($_FILES['thumbnail']['tmp_name'], $uploadfilet)) {
      ...
      $insertcontent = $db->prepare("INSERT INTO contents SET
		...
		$save_content_insert = $insertcontent->execute(array(
		     "music", $md5musicname, $getmusicname, " ", $getthumbnailname
		));
      ...
      if($save_content_insert) {
		...
		echo "Müzik yüklenirken hata oluştu! <br>";
	};
}
...
header("Location: ../index.php");
    ...
    header("Location: index.php?redir=admin/upload_music.php");
}
?>
...
<html lang="tr">
  <head>
    ...
    <meta property="og:description" content="Daha iyi bir eğitim NOMEE6 Eğitim ile mümkün! Sadece 3 ders ile tüm nesili gelecee çok iyi hazırlıyoruz." />
	<?php
	echo("
	<!-- Matomo -->
	  <script>
							...
							<div class="col-xl-4">
				            	<div class="row">
									<div class="col-md-6 col-xl-12">
										<div class="mb-3">
                                            <input type="text" placeholder="Müzik İsmi" name="musicname" id="musicname" class="form-control" required>
                                        </div>
                                        <div class="mb-3">
                                            <div class="form-label">Müzik</div>
                                            <input type="file" accept=".mp3" name="musicupload" id="musicupload" class="form-control" required>
                                        </div>
                                        <div class="mb-3">
                                            <div class="form-label">Müzik Kapağı</div>
                                            <input type="file" accept="image/*" name="thumbnail" id="thumbnail" class="form-control" required>
                                        </div>
                                        <div class="input-group">
                                            <input type="submit" value="Ykle" name="submit" class="btn">
                                        </div>
								        </div>
							          </div>
						            </div>
