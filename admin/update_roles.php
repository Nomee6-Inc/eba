...
if($getuserid == "aliyasin") {
    header("Location: panel.php");
};
if (isset($_POST['submit'])) {
    $getnewperm = $_POST['perm'];
    $datao = file_get_contents('../userdata.json');

    $json_arro = json_decode($datao, true);

    foreach ($json_arro as $keyo => $valueo) {
        if ($valueo['id'] == $_GET['userid']) {
            $json_arro[$keyo]['role'] = $_POST['perm'];
        }
        if ($valueo['id'] == $_GET['userid']) {
            if($_POST['perm'] == "Öğrenci") {
                $getrolecolor = "purple";
            } else if($_POST['perm'] == "Yönetici") {
                $getrolecolor = "green";
            };
            $json_arro[$keyo]['rolecolor'] = $getrolecolor;
        }
    }
