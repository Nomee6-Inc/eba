...
if(isset($_POST["submit"])) {
  ...
  $uploadfilet = $uploaddirt . basename($_FILES["thumbnail"]["name"]);
	...
	function fileWriteAppend(){
		$current_data = file_get_contents('../videos.json');
		$array_data = json_decode($current_data, true);
		$extra = array(
			 'videoid'               =>     md5($_POST['videoname']),
             'name'          =>     $_POST['videoname'],
			 'videothumbnail'          =>     basename($_FILES["thumbnail"]["name"]),
		);
		$array_data[] = $extra;
...
if(file_exists('../videos.json'))
{
     ...
     if(file_put_contents('../videos.json', $final_data))
     {
          $message = "success";
     }
}
