...
$getuserlooklater = $dataquery['look_later'];
  ...
  if(isset($_POST['save_later'])) {
  	$get_save_later_content = htmlentities($_POST['save_later']);
    
    if($getuserlooklater != "") {
    	  $new_user_look_later = $getuserlooklater.",".$get_save_later_content;
    } else {
    	$new_user_look_later = $get_save_later_content;
    }
	...
	$look_later_list_update = $query->execute(array(
     "new_look_later" => "$new_user_look_later",
    ...
    header("Location: index.php?redir=docs.php");
}
?>
...
<html lang="tr">
  <head>
    ...
    <title>Dökümanlar | Escort Buluşma Ağı</title>
    <!-- CSS files -->
    ...
    <meta property="og:description" content="Daha iyi bir eğitim NOMEE6 Eğitim ile mümkün! Sadece 3 ders ile tüm nesili gelecee ok iyi hazırlıyoruz." />
	<?php
	echo("
	<!-- Matomo -->
	  <script>
  ...
  <body>
    <div class="wrapper">
      ...
      <div class="page-wrapper">
        ...
        <div class="container-xl">
            <form class="row row-cards" action="" method="POST">
              <?php
             $sql = "SELECT * FROM contents WHERE type = 'doc'";
             $result = mysqli_query($conn, $sql);
             while($row = mysqli_fetch_array($result)){
               $getcontentid = $row['contentid'];
               $getcontentthumbnail = $row['thumbnail'];
               $getcontentname = $row['name'];
               
               if(strstr($getuserlooklater, "$getcontentid")) {
               		$getdisabledlooklater = "disabled";
               } else {
               		$getdisabledlooklater = "";
               }
               
                echo "
                <div class=\"col-sm-6 col-lg-4\">
                	<div class=\"card card-sm\">
                    	<a href=\"view_doc.php?docid=$getcontentid\" class=\"d-block\"><img src=\"assets/$getcontentthumbnail\" class=\"card-img-top\"></a>
                        	<div class=\"card-body\">
                        		<div class=\"d-flex align-items-center\">
                                <div>
                                <div>$getcontentname</div>
                                <br>
									<button name=\"save_later\" value=\"$getcontentid\" class=\"btn\" $getdisabledlooklater>
