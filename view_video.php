...
$getvideoid = htmlentities($_GET['videoid']);
...
if(isset($_POST['addcomment'])) {
  	$getcommentcontent = htmlentities($_POST['comment']);
  	$getdate = date("d M Y, H:i A")." GMT+3";
	$savecomment = $db->prepare("INSERT INTO comments SET
			content = ?,
			comment = ?,
			user = ?,
            date = ?");
	$save_comment_query = $savecomment->execute(array(
		     "$getvideoid", "$getcommentcontent", "$get_user_unique_id", "$getdate"
	));
	if ( $save_comment_query ){
		header("Refresh:0");
	} else {
...
<html lang="tr">
  <head>
    ...
    <title>Video Görüntüle | Escort Buluşma Ağı</title>
    <!-- CSS files -->
    ...
    <meta property="og:description" content="Daha iyi bir eğitim NOMEE6 Eğitim ile mmkün! Sadece 3 ders ile tüm nesili geleceğe çok iyi hazırlyoruz." />
    ...
    if ('serviceWorker' in navigator) {
        ...
        console.log("Service worker bu tarayıcda desteklenmiyor.");
    }
    </script>
    <script src="https://code.jquery.com/jquery-3.5.1.js"></script>
	<?php
        ...
        <div class="container-xl">
          <div class="page-header d-print-none">
            ...
            <video
                  ...
                  autoplay
                >
                  <source src="videos/<?php echo $getvideoid; ?>.mp4" type="video/mp4" />
            </video>
            
            <div class="container-xl"><br>
              <form action="" method="POST">
              <textarea name="comment" class="form-control" placeholder="Yorum Yap..."></textarea><br>
            	<button name="addcomment" class="btn btn-primary d-none d-sm-inline-block">
            		<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
            	   Yorum Yap
            	</button>
               </form>
              <br>
            <div id="comments" class="row row-cards">
              <?php
                        $getcommentsquery = "SELECT * FROM comments WHERE content = '$getvideoid'";
                        $getcommentsresult = mysqli_query($conn, $getcommentsquery);
                        while($row = mysqli_fetch_array($getcommentsresult)){
                          	$getcommentouser = $row['user'];
                          	$getcomment = $row['comment'];
                          	$getcommentdate = $row['date'];
                          	$query1 = mysqli_query($conn, "SELECT * FROM users WHERE userid = '$getcommentouser'");
							$result1 = $query1->fetch_assoc();
                          	$getcommentouname = $result1['username'];
                          	$getcommentoavatar = $result1['avatar'];
                            echo "<div class=\"col-sm-6 col-lg-4\">
                            <div class=\"card card-sm\">
                            <div class=\"card-body\">
                            <div class=\"d-flex align-items-center\">
                            <div>
                            <span class=\"avatar\" style=\"background-image: url(./avatars/$getcommentoavatar)\"></span>
                            <a href=\"profile.php?profileid=$getcommentouname\">$getcommentouname</a>
                            <div><h3>Yorum: $getcomment<h3></div>
