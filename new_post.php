...
$getpostcontent = htmlentities($_POST['postdesc']);
$getdate = date("d M Y, h:i A ")."GMT+3";
$savepost = $db->prepare("INSERT INTO posts SET
			user = ?,
			post = ?,
			date = ?");
$run_savepost = $savepost->execute(array(
	"$get_user_unique_id", "$getpostcontent", "$getdate"
));
if($savepost) {
	echo "Post başarıyla paylaşıldı.";
    ...
    header("Location: index.php?redir=new_post.php");
}
?>
...
<html lang="tr">
  <head>
    ...
    <title>Post Paylaş | Escort Buluşma Ağı</title>
    <!-- CSS files -->
    ...
    <meta property="og:description" content="Daha iyi bir eğitim NOMEE6 Eitim ile mümkün! Sadece 3 ders ile tüm nesili geleceğe çok iyi hazırlıyoruz." />
